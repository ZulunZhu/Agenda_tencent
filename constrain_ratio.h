 ...
 public:
  ...
  void calculate_para(arma::mat x){
    if(config.algo == LAZYUP){  
      ...
      complexity[0] = 1/config.rmax;
      complexity[1] = config.edges*config.rmax*config.omega;
      tau[0] = qtau1/complexity[0];
      tau[1] = qtau2/complexity[1];
      tau[2] = utau1/complexity[1];
    }
  ...
  double Evaluate(const arma::mat& x){
    ...
    if(config.algo == LAZYUP){
      ...
      double t_q = complexity[0]/x(0,0)*tau[0]+complexity[1]*x(0,0)*tau[1];
      double t_u = complexity[1]*x(0,0)*tau[2];
      double fx; 
      if(1-config.lambda_u*t_u-config.lambda_q*t_q>0){
  ...
  void Gradient(const arma::mat& x, arma::mat& g){
    if(config.algo == LAZYUP){
      ...
      // g = {-complexity[0]/x(0,0)/x(0,0)*tau[0]+complexity[1]*tau[1],0};
      ...
      double dt_q =  complexity[0]/dbeta1(0,0)*tau[0]+complexity[1]*dbeta1(0,0)*tau[1];
      double dt_u = complexity[1]*dbeta1(0,0)*tau[2];
  ...
  void GradientConstraint(const size_t i, const arma::mat& x, arma::mat& g){
    if(config.algo == LAZYUP){
      ...
      switch (i){
        ...
        case 0: g = {config.lambda_u*(complexity[1]*tau[2])+config.lambda_q*(-complexity[0]/x(0,0)/x(0,0)*tau[0]+complexity[1]*tau[1]),0};
          break;
        case 1: g = {-1,0};
          break;
...
pair<double,double> improve_throughput()
{
  ...
  if(config.algo == LAZYUP){
    ...
    queryfile << "function is " <<f.complexity[0]*tau[0]<<" "<< f.complexity[1]*tau[1]<<endl;
    queryfile << "Optimal X is " << x<<endl;
    queryfile << "original fx " << f.Evaluate(original) <<" "<<endl;
    queryfile << "Minimum of the function is " << f.Evaluate(x)<<endl;
  }
