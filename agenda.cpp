        ...
        if(config.test_throughput == true){
            ...
            for(int i = 0; i < config.runs; i++){
                ...
                // start the orinial and optimization
                 ...
                 // workload
                vector<Query> list_query;
                vector<Query> list_update;
                vector<pair<int,int>> updates;
                crowd_flag = 0;
                if(config.time_sequence==false){
                    list_query = generate_query_workload_with_timestamp(config.lambda_q, config.simulation_time, DQUERY,graph);
                
                    list_update = generate_query_workload_with_timestamp(config.lambda_u, config.simulation_time, DUPDATE,graph);
                    merge_dynamic_workload(list_query, list_update);
                    // cout << "listqqqq:" << list_query[1000].init_time<< "listuuu"<< list_update.size()<<endl;
                
                    regenerate_updates(graph, updates,list_update);

                    // **Generate the samples of the time sequences

                    // string filename = config.graph_location + "query_sequence.txt";
                    // ofstream queryfile(filename);
                    // for(int i=0; i<list_query.size(); i++){
                        
                    //     queryfile<<list_query[i].node_id<<" "<<list_query[i].init_time<<endl;
                    // }

                    // filename = config.graph_location + "update_sequence.txt";
                    // ofstream updatefile(filename);
                    // for(int i=0; i<list_update.size(); i++){
                        
                    //     updatefile<<updates[i].first<<" "<<updates[i].second<<" "<<list_update[i].init_time <<endl;
                    // }

                }else{      
                    string query_stream = config.graph_location + "query_sequence.txt";
                    assert_file_exist("query sequence", query_stream);
                    FILE *fin_q = fopen(query_stream.c_str(), "r");
                    int node; 
                    char time_q[100];
                    ...
                    string update_stream = config.graph_location + FILESEP + "update_sequence.txt";
                    assert_file_exist("update sequence", update_stream);
            ...
            if(config.algo == LAZYUP){
                queryfile_sum<<"Algorithm: "<< config.algo<<config.with_rw_idx<< " time window: "<< config.simulation_time <<" Query rate: "<<config.lambda_q<<
